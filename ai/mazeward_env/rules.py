...
class EnvConfig:
    ...
    #: 1 島に同時に存在できる敵の上限。**実ゲームには無い制限**で、
    #: 配列を固定長で確保するためだけのもの（観測は敵ごとのスロットではなく
    #: ヒートマップなので、ここを増やしても観測の形は変わらない）。
    #: 上限に当たると湧きが**黙って捨てられ**、送り手はコインを払った損だけ
    #: 残る。捨てられた敵は漏れないので**守りが強く見える**
...
#: **``attacker_limit`` は「安い順に何種類まで使えるか」。**
#: 梯子が 18 段（インカム 16 + 削り切る用 2）に伸びたので、段数と試合時間の
#: 対応を取り直してある。解禁は経済側でも 75〜90 秒に 1 回ずつ進むので、
#: 試合時間より多い段数を許しても実際には触れない。
#: 詳しくは docs/VERSUS_ECONOMY_ja.md
CURRICULUM = (
    # (名前, board_size, players_choices, attacker_limit, max_ticks)
    ("基礎 2人 15x15 走狗のみ 4分", 15, (2,), 2, 20 * 60 * 4),
    ("拡張 2人 17x17 序盤 6分", 17, (2,), 5, 20 * 60 * 6),
    ("中盤 2-4人 21x21 10段 10分", 21, (2, 3, 4), 10, 20 * 60 * 10),
    ("多人数 2-6人 21x21 14段 14分", 21, (2, 3, 4, 5, 6), 14, 20 * 60 * 14),
    ("完全 2-8人 21x21 全種 20分", 21, (2, 3, 4, 5, 6, 8), 18, 20 * 60 * 20),
)
