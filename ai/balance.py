...
class Trait:
    ...
    revives: int = 0                # 復活: 出発点へ戻れる回数（ライフ上限を奪う）
...
# ════════════════════════════════════════════════════════════════════
#: 撃破報酬の比率 (AttackerKind#killReward)
KILL_REWARD_RATIO = 0.20
...
class AttackerDef:
    ...
    def kill_reward(self) -> int:
        """撃破報酬（コイン）。送りコストの 20%。"""
        return max(1, round(self.cost * KILL_REWARD_RATIO))


ATTACKERS: Dict[str, AttackerDef] = {
    "WHELP":     AttackerDef("走狗",   "GRUNT",      15, 1, 1,   0,   40),
    "DASHER":    AttackerDef("疾走者", "RUNNER",     25, 1, 1,  15,   30),
    "SAPPER":    AttackerDef("妨害者", "SAPPER",     40, 2, 2,  30,   70),
    "STONEBACK": AttackerDef("石背",   "BRUTE",      45, 2, 2,  40,  170),
    "SKIMMER":   AttackerDef("浮遊蟲", "FLYER",      60, 2, 2,  70,   95),
    "PHASER":    AttackerDef("瞬移体", "BLINKER",    70, 2, 2,  90,  110),
    "CHANTER":   AttackerDef("祈祷師", "HEALER",     80, 3, 2, 110,  130),
    "CLEAVER":   AttackerDef("分裂体", "SPLITTER",   95, 3, 3, 140,  150),
    "CINDER":    AttackerDef("熱塊",   "EMBERLING", 120, 3, 3, 180,  175),
    "BULWARK":   AttackerDef("庇護者", "AEGIS",     150, 4, 3, 230,  260),
    "REAPER":    AttackerDef("終焉騎", "REAPER",   400, 0, 4, 300,  420),
    "CALAMITY":  AttackerDef("災厄",   "BOSS",      400, 0, 5, 350, 6600),
}
...
TOWERS: Dict[str, TowerDef] = {
    ...
    "WATCHTOWER": TowerDef(
        ...
        effect=Effect(boost_damage=0.30, boost_rate=0.15),
...
#: 強化の最大段階 (TowerKind.MAX_LEVEL)
MAX_TOWER_LEVEL = 3
#: 売却時の返金率 (TowerInstance#sellValue)
SELL_REFUND = 0.6

# --- 強化の伸び方 (TowerKind#statsAt / #upgradeCost) -----------------
UPGRADE_COST_BASE = 0.7        # cost = base_cost * (0.7 + 0.55 * (level + 1))
UPGRADE_COST_STEP = 0.55
LEVEL_DAMAGE_STEP = 0.55       # levelMul = 1 + 0.55 * level
...
def _stats_with(d: TowerDef, level: int, spec: Optional[int]) -> TowerStats:
    ...
    level_mul = 1.0 + LEVEL_DAMAGE_STEP * level
    ...
    chain = d.chain_targets + (1 if level >= 2 else 0)
...
def upgrade_cost(tower: str, level: int) -> int:
    ...
    return round(base * (UPGRADE_COST_BASE + UPGRADE_COST_STEP * (level + 1)))
...
class Economy:
    ...
    income_interval: int = 20 * 10       # 基準の収入間隔
    min_income_interval: int = 20 * 5    # 少人数でもここより速くしない
    full_lobby: int = 7                  # ここ以上の人数で基準どおり
    income_step_per_player: int = 20     # 生存者 1 人につき縮む tick
    stock_interval: int = 20             # ストックが 1 回復する間隔
    sudden_death_ticks: int = 20 * 60 * 15
    sudden_death_leak: int = 2           # サドンデス後のリークダメージ
    normal_leak: int = 1
    reaper_max_life_steal: int = 1       # 終焉騎が奪うライフ上限
    ...
    def income_interval_for(self, alive: int) -> int:
        """生存者数に応じた収入間隔。 (VersusMatch#incomeInterval)

        少人数ほど「送りが自分に届く数」が減って撃破報酬が細るので、
        定期収入を速くして補っている。**人数で最適戦略が変わる最大の要因**。
        """
        a = max(2, min(self.full_lobby, alive))
        interval = self.income_interval - (self.full_lobby - a) * self.income_step_per_player
        return max(self.min_income_interval, interval)
...
class Balance:
    ...
    def kill_reward(self, attacker: str) -> int:
        return max(1, round(self.attackers[attacker].cost * KILL_REWARD_RATIO))
...
def randomized_balance(rng, strength: float = 0.20) -> Balance:
    ...
    ec.min_income_interval = min(ec.min_income_interval, ec.income_interval)
...
__all__ = [
    ...
    "MAX_TOWER_LEVEL", "SELL_REFUND", "TICKS_PER_SECOND", "KILL_REWARD_RATIO",
    "KILL_INCOME_RATIO", "DISABLE_REFRESH_TICKS", "HEAL_AURA_RADIUS",
    "HEAL_INTERVAL", "WARD_TICKS", "SPLIT_HP_RATIO", "SPLIT_REWARD_DIVISOR",
    ...
    "MIN_COOLDOWN", "UPGRADE_COST_BASE", "UPGRADE_COST_STEP",
    "LEVEL_DAMAGE_STEP", "LEVEL_RANGE_STEP", "LEVEL_COOLDOWN_MUL",
